...
> `203.0.113.10`，跑通 `curl /api/health` + 真实扫描 `result.source=real`。

## 0. 阻塞前置：SSH 访问

服务器 `203.0.113.10` 仅接受公钥认证（`PermitRootLogin prohibit-password` +
...
- **B. 用户在 Claude Code 用 `! ssh root@203.0.113.10` 走自己已认证的会话**，剩余命令逐条发给我执行。
...
ssh root@203.0.113.10 'cd /opt/attrax && git pull --rebase origin main'

# FAISS 数据（如本地构建）
rsync -avz --progress -e ssh data/faiss/ root@203.0.113.10:/opt/attrax/data/faiss/
```
...
```bash
ssh root@203.0.113.10 'cd /opt/attrax && \
...
```bash
curl -s https://203.0.113.10/api/health | python -m json.tool
```
...
# 发起扫描
SID=$(curl -s -X POST http://203.0.113.10:3000/api/scan \
...
# 轮询（间隔 10s，限流）
curl -s http://203.0.113.10:3000/api/scan/<sessionId> \
...
```bash
# 快速诊断
ssh root@203.0.113.10 'grep -E "rag-service call failed|Invalid input|mimoTalk" /var/log/attrax-next.err.log | tail -20'
ssh root@203.0.113.10 'tail -30 /var/log/attrax-rag.err.log'
```
...
```bash
# 回到上一次成功 build
ssh root@203.0.113.10 'cd /opt/attrax && git stash && git log --oneline -5'

# 用备份还原数据
ssh root@203.0.113.10 'ls /opt/attrax/backups/'   # 选最近 tar.gz
ssh root@203.0.113.10 'cd /opt/attrax && tar -xzf backups/attrax-data-LATEST.tar.gz'
```
