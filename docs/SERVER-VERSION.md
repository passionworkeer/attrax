...
> ⚠️ **SUPERSEDED — 2026-08-13 历史快照**。本文档仅适用已退役的阿里云深圳服务器 `120.77.36.107`。attrax 当前生产已迁至腾讯云首尔 lighthouse（`43.155.141.192`）；`.deployed` 治本机制对 lighthouse 同样适用，对账方式见根目录 [`CLAUDE.md`](../CLAUDE.md) §部署雷区 与 [`docs/README.md`](./README.md) §生产部署。

> 服务器当前跑的是哪份代码?跟哪个 git ref 一致?
