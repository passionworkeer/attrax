...
## 当前活跃文件（aliyun-sz，阿里云深圳 120.77.36.107）
...
## 历史快照（⚠️ lighthouse 时代，`43.155.141.192` 已退役）

以下文件是 2026-06-21 从旧机抓取的快照，**不要直接 scp 到 aliyun-sz**；仅作新机参考。
...
> `attrax-404-probe` jail 未部署到 aliyun-sz；aliyun-sz 实际有 4 个 jail（迁回后需 `fail2ban-client status` 复核）
...
ssh aliyun-sz 'sudo cp /tmp/nginx-attrax-locations.conf /etc/nginx/snippets/attrax-locations.conf && sudo nginx -t && sudo systemctl reload nginx'

# cron —— 注意安装时要改名：仓内 cron-attrax-backup → /etc/cron.d/attrax-backup
# （与服务器现有命名一致；/etc/cron.d 只接受 注释 / KEY=VALUE / 作业行）
scp docs/infra/cron-attrax-backup docs/infra/cron-attrax-backup-remote aliyun-sz:/tmp/
ssh aliyun-sz 'sudo install -m 644 -o root -g root /tmp/cron-attrax-backup /etc/cron.d/attrax-backup'
ssh aliyun-sz 'sudo install -m 644 -o root -g root /tmp/cron-attrax-backup-remote /etc/cron.d/attrax-backup-remote'
ssh aliyun-sz 'ls -la /etc/cron.d/'

# logrotate —— 装完务必先 dry-run
scp docs/infra/logrotate-attrax aliyun-sz:/tmp/
ssh aliyun-sz 'sudo cp /tmp/logrotate-attrax /etc/logrotate.d/attrax && sudo logrotate -d /etc/logrotate.conf 2>&1 | grep -i error; echo "dry-run exit=$?"'
ssh aliyun-sz 'sudo systemctl status logrotate --no-pager | head -5'

# 脚本（单一来源 = 本仓 scripts/）
scp scripts/backup-data.sh scripts/backup-remote.sh scripts/apply-deploy.sh aliyun-sz:/tmp/
ssh aliyun-sz 'sudo install -o ubuntu -g ubuntu -m 700 /tmp/backup-data.sh /tmp/backup-remote.sh /opt/attrax/scripts/ && sudo install -m 755 /tmp/apply-deploy.sh /tmp/attrax-apply-deploy.sh'
```
