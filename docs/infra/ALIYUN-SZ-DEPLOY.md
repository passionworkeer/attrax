...
> **2026-09-18 起生效**：attrax 整站部署到 `aliyun-sz`（深圳 120.77.36.107）。lighthouse（首尔）上的 attrax 进程已停 + nginx 站点已卸，`/opt/attrax` 数据原地保留。
>
...
| 主机 | SSH 用户 | IP | 跑什么 |
|---|---|---|---|
| **aliyun-sz**（生产） | root（`id_ed25519`） | 120.77.36.107 | attrax nextjs:3000 + rag-service:8001 + regwatch；nginx 80→443，HTTPS 复用 twinbuddy 证书。端口单一来源 = `scripts/ports.env`（见 §4.4） |
| **lighthouse**（仅 portfolio/study/monitor） | ubuntu（`lighthouse_seoul_new`） | 43.155.141.192 | portfolio nextjs:3002 + study/monitor 静态站。**attrax 不再跑** |

实例 `Ubuntu-lrtz`（`e499f9de56204c108b334a81e675cade`）2026-10-11 到期，**必须续费**（这是 attrax 的生产机）。
...
- **证书**：复用 `/etc/letsencrypt/live/twinbuddy.xyz/`，server_name = `twinbuddy.xyz www.twinbuddy.xyz 120.77.36.107 wangjianjun.xyz`
...
curl -skS -o /dev/null -w "HTTP %{http_code}\n" https://120.77.36.107/api/health   # 证书域名不匹配警告但可用
```
...
server {  # HTTP → HTTPS redirect
    listen 80; listen [::]:80;
    server_name twinbuddy.xyz www.twinbuddy.xyz 120.77.36.107 wangjianjun.xyz;
...
server {  # HTTPS
    ...
    server_name twinbuddy.xyz www.twinbuddy.xyz 120.77.36.107;
...
**绝不要**：
- 在 git 里写真值（旧值 `f776977a` / `a161b8b0` 在 git 历史暴露过，已全部失效）
...
2. **`wangjianjun.xyz` DNS 切换**：域名注册商改 A 记录 `120.77.36.107` + 阿里云安全组放行 80（已开）+ certbot 申请 `wangjianjun.xyz` 证书 + 替换 nginx `ssl_certificate` 路径
...
5. **访问域名 cert 不匹配告警** 用户测试时给指引（`twinbuddy.xyz` 无 warning，`wangjianjun.xyz` 有 warning）
