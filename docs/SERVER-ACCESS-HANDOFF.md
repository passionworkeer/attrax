...
- Private-key path: `C:\workspace\.ssh\id_ed25519` (never copy this file into the repository)
- Authorized public-key fingerprint: `SHA256:SHA256:REDACTED_PUBKEY_FINGERPRINT`
- Server user: `admin`
...
```powershell
ssh -o BatchMode=yes -o ConnectTimeout=10 admin@203.0.113.10 "whoami; hostname; cd /opt/attrax && pwd"
```

Expected identity and path: `admin`, host `iZinstance0000001`, `/opt/attrax`.
...
On the server, first back up the authorization file, then remove the line whose comment is `188482362+passionworkeer@users.noreply.github.com`:

```bash
cp /home/admin/.ssh/authorized_keys \
  /home/admin/.ssh/authorized_keys.bak.$(date +%Y%m%d-%H%M%S)
sed -i '/188482362+passionworkeer@users.noreply.github.com$/d' /home/admin/.ssh/authorized_keys
