...
- **生产**：腾讯云首尔 lighthouse `198.51.100.20`，pm2 跑 `nextjs` + `rag-service` + `regwatch`，nginx 反代，**不走 docker-compose / Ansible**（`docs/infra/` 下的 Ansible 文件是历史 aliyun-sz 时代遗留）。
