...
> 这台机器怎么连到 `203.0.113.10`，写下来免得每次都翻聊天记录。
...
| 本地私钥 | `C:\workspace\.ssh\id_ed25519` |
| 公钥 | `C:\workspace\.ssh\id_ed25519.pub` |
| 部署目录 | `/opt/attrax/` |
...
```bash
ssh -i /c/Users/workspace/.ssh/id_ed25519 admin@203.0.113.10
```
...
```bash
ssh -i /c/Users/workspace/.ssh/id_ed25519 -o StrictHostKeyChecking=no \
    admin@203.0.113.10 'cd /opt/attrax && ls -la .next/standalone/.next/BUILD_ID'
```
