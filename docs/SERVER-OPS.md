...
> **当前生产部署**: Alibaba Cloud ECS `203.0.113.10`,无 Docker 部署,Node 22 + Python 3.12 venv + pm2 + systemd。
> 最后更新: 2026-06-21
...
```bash
# 健康检查 (前端 + RAG 状态)
curl http://203.0.113.10:3000/api/health
...
```
┌─────────────────────────────────────────────────────┐
│ 公网:203.0.113.10:3000 (Next.js standalone)         │
...
# 然后轮询
curl http://203.0.113.10:3000/api/scan/<sessionId> \
...
# 如果丢了,从构建机器同步:
scp /opt/attrax/data/faiss/legal_chunks.index root@203.0.113.10:/opt/attrax/data/faiss/
scp /opt/attrax/data/faiss/legal_chunks_meta.json root@203.0.113.10:/opt/attrax/data/faiss/
