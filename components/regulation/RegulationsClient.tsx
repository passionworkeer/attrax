...
import styles from "@/app/regulations/regulations.module.css";
import { ArchiveTab } from "@/app/regulations/ArchiveTab";
import { SourcesTab } from "@/app/regulations/SourcesTab";
import { UpdatesTab } from "@/app/regulations/UpdatesTab";
...
export default function RegulationsClient({ initialStats }: { initialStats: TopStats }) {
  ...
  const [stats, setStats] = useState<TopStats>(initialStats);
  const [statsFailed, setStatsFailed] = useState(false);

  // 用 server 端传入的 initialStats 渲染首屏（数字真实可见）；
  // useEffect 仅作为刷新机制，当用户在浏览器里手动重拉数据时更新。
  // 避免首屏被 Next.js 当作 SSG fallback shell 缓存（s-maxage=1y）。
  useEffect(() => {
