  ...
  return <main className={`${bright.page} ${upload.page} ${styles.page} complipilot-flow`}>
    ...
    <div className={styles.content}>
      ...
      <section className={`${styles.scorePanel} blaze-panel`} data-section="assessment-score" aria-label={zh?"AI 合规评估":"AI compliance assessment"}>
        ...
        {priorityRows.length>0&&<div className={styles.priorityArea}><button data-priority-toggle type="button" className={`${styles.critical} ${styles.criticalButton}`} aria-expanded={priorityOpen} aria-controls="priority-issues" onClick={()=>setPriorityOpen(open=>!open)}><span>{zh?`${priorityRows.length} 项问题需核对处理，总分不能抵消这些问题。`:`${priorityRows.length} issues require review and action regardless of score.`}</span><strong>{priorityOpen?(zh?"收起问题清单":"Hide issues"):(zh?"查看具体问题":"Show issues")}<ChevronDown className={styles.priorityChevron} data-open={priorityOpen} size={15} aria-hidden="true"/></strong></button><div id="priority-issues" className={styles.priorityReveal} data-open={priorityOpen} aria-hidden={!priorityOpen}><div className={styles.priorityRevealInner}><ol className={styles.priorityList}>{priorityRows.map(row=><li key={row.check.checkId}><button tabIndex={priorityOpen?0:-1} type="button" onClick={()=>focusPriority(row.check.checkId)}><span className={styles.priorityCopy}><strong>#{row.number} · {checkLabel(row.check.checkId,locale,row.check.title)}</strong><span>{row.claim?.reason||observationSummary(row,locale)}</span></span><span className={styles.jumpCue}>{zh?"定位检查":"Jump to check"}<ArrowUpRight size={13} aria-hidden="true"/></span></button></li>)}</ol></div></div></div>}
        <p className={styles.coverageLine}>{zh?`有据支持 ${assessment.supported} 项 · 需处理 ${assessment.blocked} 项 · 适用判断未完成 ${assessment.unresolved.length} 项`:`Supported ${assessment.supported} · Issues ${assessment.blocked} · Unresolved ${assessment.unresolved.length}`}</p>
      ...
      <details className={`${styles.reading} blaze-panel`}><summary>{zh?"违规后果与潜在处罚":"Potential consequences & penalties"}</summary><p>{zh?penalty.text:penalty.en}</p>{penalty.source&&<a href={penalty.source} target="_blank" rel="noreferrer">{penalty.article} · {zh?"官方依据":"Official source"}</a>}</details>
      ...
      <details id="reports" className={`${styles.reading} blaze-panel`}><summary>{zh?"完整报告与下载 · 全部市场":"Full report & downloads · all markets"}</summary><ComplianceReportView result={enrichedReport} documentOnly/></details>
...
function EvidenceInputs({result,locale}:{result:ScanResult;locale:"zh"|"en"}) {
  ...
  return <details className={`${styles.reading} blaze-panel`}><summary>{zh?"本次分析使用的补充材料":"Supplementary inputs used in this analysis"}</summary>
