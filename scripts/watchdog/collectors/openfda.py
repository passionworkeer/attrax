...
def _normalize_recall(recall: dict) -> dict | None:
    """Pull the stable fields from one device recall record.

    Tolerates both the public schema (``recall_number`` /
    ``recall_initiation_date``) and a couple of legacy aliases.
    """
    recall_number = str(
        recall.get("recall_number") or recall.get("cfres_id") or ""
    ).strip()
    ...
    return {
        ...
        "classification": str(recall.get("product_classification") or "").strip(),
