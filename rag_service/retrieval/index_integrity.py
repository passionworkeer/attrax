...
def inspect_index_bundle(
    ...
    missing = [str(path) for path in (index, metadata, manifest) if not path.is_file()]
    if missing:
...
def validate_index_bundle(
    ...
    *,
    ...
    snapshot = inspect_index_bundle(index_path, metadata_path, manifest_path)
    manifest = _load_json(snapshot.manifest_path)
    ...
    for key, actual in expected.items():
        ...
        if manifest_value != actual:
            raise IndexIntegrityError(
                f"manifest {key} mismatch: expected {actual}, got {manifest_value!r}"
            )

    if require_hashes:
        if manifest.get("schema_version") != MANIFEST_SCHEMA_VERSION:
            raise IndexIntegrityError("manifest is not sealed with schema version 2")
        if manifest.get("index_sha256") != snapshot.index_sha256:
            raise IndexIntegrityError("FAISS index hash does not match manifest")
        if manifest.get("metadata_sha256") != snapshot.metadata_sha256:
